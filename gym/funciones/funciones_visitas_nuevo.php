...
function guardar_nuevo_dia()
{
    ...
    $id_socio         = request_var('id_socio', 0); // Obtener el ID del socio
    ...
    if ($stmt) {
        ...
        if ($resultado_insert && $id_visita && $token) {
            ...
            if ($metodo_pago == 'M') {
                ...
                $incremento = $hor_monedero * ($porcentaje_incremento / 100);
                ...
                if ($stmt_monedero) {
                    ...
                    if ($resultado_monedero) {
                        ...
                        Aumento por porcentaje de incremento: $" . number_format($incremento, 2) . "<br>
                ...
                // Inserción exitosa (sin necesidad de actualizar monedero)
                $exito['num'] = 1;
                $exito['msj'] = "Guardado.";
                $exito['IDV'] = $id_visita;
                $exito['tkn'] = $token;
                ...
                $correo_socio = obtener_correo_socio($id_socio);
                $asunto = "Confirmación de visita";
                $mensaje = "Estimado socio, su visita ha sido registrada exitosamente.";
                enviar_correo($correo_socio, $asunto, $mensaje);
            }
        } else {

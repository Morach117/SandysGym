<?php
	...
	if( $enviar )
	{
		...
		if( $chk_prepago )
		{
			$pet_saldo = obtener_saldo_socio( $id_prepago );
		...
		if( $continuar )
		{
			if( $js_metodo_pago == 'E' || $js_metodo_pago == 'T' )
			{
				if( $count_ids == $count_cants && is_numeric( $js_tot_efectivo ) && is_numeric( $js_tot_prepago ) )
				{
					$query			= "	SELECT		art_id_articulo AS id_articulo,
										...
										WHERE		art_id_articulo IN ( $ids_articulo )
					...
					if( $continuar && ( ( $js_tot_a_pagar == $v_sub_total && $js_metodo_pago == 'E' ) || ( $js_metodo_pago == 'T' && $js_tot_a_pagar == $v_tot_tarjeta ) ) )
					{
						if( $js_metodo_pago == 'E' )
						...
						if( !$mensaje )
						{
							if( ( $v_efectivo_prep >= $v_sub_total && $js_metodo_pago == 'E' ) || $js_metodo_pago == 'T' )
							{
								if( $commit == 'S' )
								{
					...
					$mensaje .= "<li>No se escribieron cantidades correctas para los Articulos o Efectivo.</li>";
			}
			else
